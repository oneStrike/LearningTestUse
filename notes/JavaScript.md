...
如果左边用于赋值的变量多余右边，那么就会赋值`undefined`,再不确定是否有值的情况下可以设置一个默认值,ES6 中采用的是严格相等,因此设置默认值的时候必须是严格等于`undefined`时才会使用默认值,同时也可以使用解构赋值的其他变量作为默认值,前提是该变量已经被声明过了,
...
> ES6 中新增的一种基本数据类型,表示独一无二的值,通过`Symbol()`构建,但是构建的时候不能使用`new`,因为`Symbol()`并不是一个构造函数,

```javascript
let mySymbol = Symbol();
```

Symbol 类型的数据在构建时可以传入一个参数,作为当前 Symbol 实例的描述,如果传入的是一个非字符串格式的,会调用`toString()`进行转换
...
> Symbol 类型的数据可以只能转换成字符串或者布尔格式,无法转换成数字,同时无法做数学运算,也无法做字符串拼接
...
> ES6 中新增加的一种遍历方法,只要当前的数据结构上部署了`[Symbol.iterator]`就可以使用`for...of`遍历,`for...of`遍历的本质就是调用数据结构的`[Symbol.iterator]`函数产生的遍历器进行遍历
...
> 是 ES6 中新增的一种数据结构,数据结构类似于数组,但是不允许出现重复的值,`Set`数据结构并不具备[key],只有[value]

`set`是一个构造函数,使用`new`来构建一个实例,初始化实例时可以传递一个数组或者具有 iterator 接口的数据结构作为参数,
...
> ES6 中新增加的一种对象的存储结构,普通对象的属性名只能是字符串格式,而`Map`的属性名可以是一个值或者说是另一个对象
